...
我们不想在物件之间建立人为的联系. 所以\emph{自然}要用现存的连接, 也就是态射. 自然变换就是选择一些态射: 对
每个物件 $a$, 它从 $F a$ 到 $G a$ 选出一个态射. 如果我们把自然变换叫做 $\alpha$, 这个态射就叫做 $\alpha$
在 $a$ 处的\newterm{分量}, 或者 $\alpha_a$.

\[\alpha_a \Colon F a \to G a\]
留意 $a$ 是 $\cat{C}$ 中的物件, 而 $\alpha_a$ 是 $\cat{D}$ 中的态射.

如果, 对某些 $a$, 在 $\cat{D}$ 中 $F a$ 和 $G a$ 之间没有态射, 那么 $F$ 和 $G$ 之间就没有自然变换.

当然, 这里只说了故事的一半, 因为函子不仅映射物件, 它们也映射态射. 所以自然变换怎么处理那些映射呢? 原来, 态射
之映射是固定的 --- 在任何 $F$ 和 $G$ 的自然映射中, $F\ f$ 必须被映射到 $G\ f$. 更进一步, 这两个函子对态射的映射
极大约束了我们定义自然变换时候的选项. 考虑 $\cat{C}$ 中两个物件 $a$ 和 $b$ 之间的态射 $f$. 它在 $\cat{D}$
中映射到两个态射: $F\ f$ 和 $G\ f$:
...
\begin{gather*}
  ...
  G f \Colon G a \to G b
\end{gather*}
自然变换 $\alpha$ 提供了两个额外的态射, 完成了范畴 \emph{D} 中的图式:
...
函子中间也不一定能有自然变换. 所以, 由自然变换关联起来的函子数目可以告诉你很多那些函子在的范畴的结构.
我们后面说到极限和 Yoneda 引理的时候, 会看到一些例子.

逐分量地看自然变换, 或许可以说它把物件映射到态射. 因为自然变换的自然性条件, 也可以说它把态射映射到交换的方形
...
最后, 自然变换可以用来定义函子之间的同构. 说两个函子是自然同构, 几乎就是说它们是一样的.
...
要构造一个自然变换, 我们开始于一个物件, 在这是个类型 \code{a}. 一个函子, \code{F}, 把它映射到类型 $F a$.
另一个函子, \code{G}, 把它映射到 $G a$. 自然变换 \code{alpha} 在 \code{a} 处的分量是一个从 $F a$ 到 $G a$
的函数. 伪 Haskell 代码如下:

\begin{snipv}
alpha\textsubscript{a} :: F a -> G a
\end{snipv}
自然变换是一个多态函数, 为所有类型 \code{a} 定义:
...
可以把 Haskell 中的函子看成泛型化的容器. 我们可以继续这个类比, 把自然变换看成把一个容器的内容转移到另一个容器的
...
看几个 Haskell 里的自然变换例子. 第一个是列表函子和 \code{Maybe} 函子之间的自然变换. 它返回列表的头部, 但只有
列表不为空的时候才返回:

\src{snippet04}
这是个 \code{a} 里的多态函数. 它对任何类型 \code{a} 都有效, 没有任何限制条件. 所以这是个参数多态的例子. 因此它
是两个函子中间的自然变换. 但为了说服我们自己, 一起验证一下自然性条件.
...
\src{snippet11}
有意思的情况是, 其中一个函子为平凡的 \code{Const} 函子. 出发或者到达 \code{Const} 函子的自然变换看起来就像是
一个函数, 要么对其参数类型多态, 要么对其返回类型多态.

例如, \code{length} 可以看成是从列表函子到 \code{Const Int} 函子的自然变换:
...
\src{snippet17}
对每种类型 \code{e}, 你可以定义一族自然变换, 从 \code{Reader e} 到任何其他函子 \code{f}. 我们后面会看到, 这些
...
\code{a} 中的元素数目一样多. 现在考虑从这个函子到 \code{Maybe} 函子的自然变换:
...
两个函子(包含 \code{Const} 函子这个边缘例子) 之间的参数多态函数总是一个自然变换. 既然所有标准代数数据类型都是
函子, 任何这些类型中间的多态函数自然也都是自然变换.
...
两个逆变函子中间的多态函数依然在范畴意义下是自然变换, 除了它们作用的函子在 Haskell 类型相反的那个范畴上.
...
\src{snippet23}
但是因为这两个函子不是协变的, 所以不是 $Hask$ 中的自然变换. 不过, 因为它们都是逆变的, 所以满足 ``反过来的''
自然性条件:
...
\src{snippet27}
这里 \code{f} 是任意函子. 这个函数不能是自然变换. 有意思的是, 有一个自然变换的推广, 叫做\newterm{dinatural
  transformation 双自然变换}, 处理这些情况. 我们讨论端的时候会说到这里.

\section{函子范畴}

既然我们已经有了函子之间的映射 --- 自然变换 --- 自然想问函子是否形成一个范畴. 而它们确实形成! 每一对范畴
$\cat{C}$ 和 $\cat{D}$ 之间都有一个函子的范畴. 这个范畴的物件为从 $\cat{C}$ 到 $\cat{D}$ 的函子, 而态射为
函子之间的自然变换.

我们需要定义两个自然变换的组合, 不过这很简单. 自然变换的分量是态射, 我们知道怎么组合态射.

确实, 来从函子 $F$ 到 $G$ 拿一个自然变换 $\alpha$. 它在物件 $a$ 处的分量是一个态射:
\[\alpha_a \Colon F a \to G a\]
我们希望把 $\alpha$ 组合到 $\beta$, 它是从 $G$ 到 $H$ 的自然变换. $\beta$ 在物件 $a$ 处的分量是一个态射:
\[\beta_a \Colon G a \to H a\]
这些态射可以组合, 其组合是另一个态射:
\[\beta_a \circ \alpha_a \Colon F a \to H a\]
我们用这个态射作为自然变换的分量.
$\beta \cdot \alpha$ --- 两个自然变换的组合, 先 $\alpha$ 再 $\beta$:
...
\noindent
看一看(可能看很久)这些图可以说服我们组合的结果确实是一个从 F 到 H 的自然变换:
...
\noindent
自然变换的组合服从结合律, 因为它们的分量, 也就是普通态射, 对组合是结合的.

最后, 对每个函子 F 都有一个单位自然变换 $1_F$, 它的分量是单位态射:
\[\id_{F a} \Colon F a \to F a\]
所以确实, 函子形成一个范畴.

说一下记号. 按照 Saunders Mac Lane 的惯例, 我用点号表示刚刚描述的自然变换组合. 问题是有两种办法组合自然变换.
...
\noindent
函子范畴 $\cat{{[}C, D{]}}$ 同样是两个范畴中间的函子集合(加上自然变换作为态射).它的物件与 $\cat{Cat(C, D)}$
...
两个物件之间的函子比集合有更丰富的结构, 他们形成一个范畴, 自然变换作为态射. 既然函子可以看成 $\Cat$ 中的态射,
自然变换就是态射之间的态射.
...
$\cat{E^C}$ 中的函子 $G \circ F$. 但我们也有在每个 Hom-范畴中的组合 --- 自然变换的垂直组合, 或者说是
函子之间 2-态射的垂直组合.
...
假设有两个自然变换 $\alpha$ and $\beta$ 分别作用在函子 $F$ 和 $G$ 上:
\begin{gather*}
  \alpha \Colon F \to F' \\
  \beta \Colon G \to G'
\end{gather*}
...
手边有了 $\alpha$ 和 $\beta$, 我们能定义一个从 $G \circ F$ 到 $G' \circ F'$ 的自然变换吗? 让我简单描述一下
...
我们还有四个态射, 它们组成方形. 两个是自然变换 $\beta$ 的分量:
\begin{gather*}
...
和 $G' \circ F'$ 的自然变换的分量. 其实不止一条路, 我们有两条路可以从 $G (F a)$ 到 $G'(F'a)$:
\begin{gather*}
...
我们刚刚定义了一个从 $G \circ F$ 到 $G' \circ F'$ 的自然变换的分量. 证明这个转换的自然性很直白, 只要你足够耐心.
...
这里有个范畴论经验之谈: 每次有组合的时候, 你都应该找一个范畴. 我们有自然变换的垂直组合, 那是函子范畴的一部分. 但
水平范畴呢? 它在哪个范畴里?

想明白这个问题的方法是从侧面看 $\Cat$. 不要把自然变换看成函子之间的箭头, 把它看成范畴之间的箭头. 自然变换在
...
\noindent
关注两个 $\cat{Cat}$ 中的物件 --- 范畴 $\cat{C}$ 和 $\cat{D}$. 有一个自然变换的集合处于连接这两个范畴的
函子中. 这些自然变换是我们从 $\cat{C}$ 到 $\cat{D}$ 的新箭头. 同样, 有自然变换在把 $\cat{D}$ 连接到 $\cat{E}$
...
既然你不能把函子同自然变换组合, 这个记号的意思就是说这个组合: $\alpha$ 之后跟着单位自然变换 $1_F$.
...
\begin{enumerate}
  \tightlist
  \item
        定义一个从 \code{Maybe} 到 \code{List} 的自然变换. 证明它的自然性条件.
  \item
        定义至少两个不同的自然变换, 它们都在 \code{Reader ()} 和列表函子中间.
        有几个不同的 \code{()} 列表?
  \item
        用 \code{Reader Bool} 和 \code{Maybe} 继续前面的练习.
  \item
        说明自然变换的水平组合满足自然性条件(提示: 用分量). 这是个画图的好练习.
  \item
